...
import { InitiativeTask, InitiativeMember } from '../../types';
...
type ManualTaskData = Omit<InitiativeTask, 'id' | 'seq'>;
...
interface ManualTaskModalProps {
    ...
    taskToManage: InitiativeTask | null;
...
const AddInitiativeTaskModal: React.FC<ManualTaskModalProps> = ({ isOpen, onClose, onSave, onDelete, taskToManage, members }) => {
        ...
        if (isOpen) {
            if (taskToManage) {
                setFormState({
                    ...
                    assignee: taskToManage.assignee || '',
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormState(prev => ({
            ...prev,
            [name]: (name === 'actual_percent') ? Number(value) : value,
        }));
    };
                ...
                {viewMode === 'view' && taskToManage ? (
                    <div className="p-6 overflow-y-auto space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-natural-200 dark:border-natural-700">
                            ...
                            <DetailItem label={t('initiatives.details.timeline.assignee')} value={taskToManage.assignee} />
                            <DetailItem label={t('challenges.actualProgress')} value={`${taskToManage.actual_percent}%`} />
                            <DetailItem label={t('challenges.plannedProgress')} value={`${Math.round(calculatePlannedProgress(taskToManage.start, taskToManage.end))}%`} />
                    ...
                    <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="assignee" className="block text-sm font-medium">{t('initiatives.details.timeline.assignee')}</label>
                                <input
                                    id="assignee"
                                    name="assignee"
                                    list="assignee-list"
                                    ...
                                    placeholder={t('initiatives.details.timeline.customAssignee')}
                                    className="mt-1 w-full bg-natural-50 dark:bg-natural-700 border-natural-300 dark:border-natural-600 rounded-md"
                                />
                                <datalist id="assignee-list">
                                    {members.map(member => (
                                        <option key={member.id} value={member.name[language]} />
                                    ))}
                                </datalist>
                            </div>
                            <div>
...
export default AddInitiativeTaskModal;
