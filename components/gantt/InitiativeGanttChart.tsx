...
const InitiativeGanttChart: React.FC<InitiativeGanttChartProps> = ({ tasks, onTaskClick, onAddTaskRequest }) => {
  ...
  const sortedTasks = useMemo(() => {
    const sorted = [...tasks].sort((a,b) => getUTCMidnight(a.start).getTime() - getUTCMidnight(b.start).getTime());
    // Re-assign sequence number based on sorted order for display
  ...
  const { timeScale, months, weeks, days } = useMemo(() => {
    ...
    const startDates = sortedTasks.map(c => getUTCMidnight(c.start));
    const endDates = sortedTasks.map(c => getUTCMidnight(c.end));
        ...
        <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-grow overflow-auto">
            <div className="flex min-w-max relative">
                ...
                <div className="flex-grow">
                  <div className="relative" style={{ height: sortedTasks.length * 44, width: timeScale.totalDays * dayWidth }}>
                      {days.map((day, i) => {
